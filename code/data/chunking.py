...
def creat_patchs(raw_data, patch_size, over_lap):
    ...
    if raw_data.ndim == 3:
        ...
        assert d_raw >= d_patch, 'Error: The shape of the image must be greater than or equal to the shape of the block, ' \
                                 'but are {} and {}'.format(raw_data.shape, patch_size)
...
def generate_transform_data(
    ...
    assert psf_ is None or isinstance(psf_, np.ndarray), 'Error'
    target = data
    zoom_order = 1
    ...
    if psf_ is not None:
        ...
        assert np.min(_psf) >= 0, 'Error: psf has negative values.'
        _psf /= np.sum(_psf)
...
def save_training_data(file, X, Y, Z):
    ...
    Path().expanduser()
    assert isinstance(file, (Path, string_types)), 'Error.'
...
def generate_training_data(
    ...
    assert psf is None or isinstance(psf, np.ndarray), 'Error.'
    mr_data = None
    if len(patch_size) == 3 and psf is None and lr_path != '':
        hr_list = get_file_list(path=hr_path, regx=regx)
        lr_list = get_file_list(path=lr_path, regx=regx)
        assert len(hr_list) == len(lr_list), 'Error: The two sets of data should be equal, but you ' \
                                             'provide {} and {}'.format(len(hr_list), len(lr_list))
        lr_dic = {}
        hr_dic_good = {}
        ...
        assert hr_select_array.shape[0] == lr_select_array.shape[0], (
                'Error: The amount of hr must be equal to that of lr, but are %s and %s' % (
    ...
    elif isinstance(psf, np.ndarray) and lr_path == '':
        hr_list = get_file_list(path=hr_path, regx=regx)
        lr_dic = {}
        hr_dic_good = {}
        number_hr = 0
        patch_size_sq = np.squeeze(patch_size)
        hr_patch_size = patch_size_sq[1:3] if patch_size[0] > 1 else patch_size_sq
        for i, hr_file in enumerate(hr_list):
        ...
        raise ValueError(
            "The LR path and PSF path should not exist simultaneously. Please re-enter according to the requirements!")
